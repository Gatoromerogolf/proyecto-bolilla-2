...
/* for (const puntos of puntosFedex) {
  ...
  for (let i = 0; i < 12; i++) {
    if (matrizFx [i][0] === 0){
        matrizFx[i][0] = nombreJugador;
        matrizFx[i][1] += puntos.puntosFd
        i=12}
        else{
            if(matrizFx [i][0] === nombreJugador){
                matrizFx[i][1] += puntos.puntosFd
                i=12}
            }
        }
...
/* Para crear un nuevo elemento `<td>` con un botón dentro, utilizar el método `document.createElement()`. Luego, establecer los atributos `class` y `data-info`. Aquí tienes un ejemplo:
...
boton.textContent = 'Ver detalles'; // Puedes cambiar el texto del botón aquí
